...
router.get('/area-stats', authenticateToken, requireAdmin, async (req, res, next) => {
  try {
...
router.get('/worker-stats', authenticateToken, requireAdmin, async (req, res, next) => {
  try {
...
router.get('/today', authenticateToken, requireAdmin, async (req, res, next) => {
  try {
    const today = new Date().toISOString().slice(0, 10);

    const todayDone    = await db.get(`SELECT COUNT(*) as count FROM voters WHERE status='done'    AND DATE(marked_at) = ?`, [today]);
    const todayRefused = await db.get(`SELECT COUNT(*) as count FROM voters WHERE status='refused' AND DATE(marked_at) = ?`, [today]);
