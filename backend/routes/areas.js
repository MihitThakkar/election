...
router.post('/', authenticateToken, requireAdmin, (req, res) => {
  ...
  const result = db.prepare('INSERT INTO areas (name, district) VALUES (?, ?)').run(name.trim(), district?.trim() || null);
...
router.put('/:id', authenticateToken, requireAdmin, (req, res) => {
  ...
  db.prepare('UPDATE areas SET name = ?, district = ? WHERE id = ?')
    .run(name || area.name, district !== undefined ? district : area.district, req.params.id);

  res.json({ success: true, data: db.prepare('SELECT * FROM areas WHERE id = ?').get(req.params.id) });
});
