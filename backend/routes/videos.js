...
router.post('/', authenticateToken, requireAdmin, upload.single('video'), async (req, res, next) => {
  try {
    ...
    const result = await db.run(
      ...
      [title, description || null, filePath, req.file.originalname, req.user.id]
    );

    const video = await db.get('SELECT * FROM videos WHERE id = ?', [result.insertId]);
