...
async function initDatabase() {
  ...
  try { _db.exec('ALTER TABLE voters ADD COLUMN part_number INTEGER'); } catch {}
  try { _db.exec('CREATE INDEX IF NOT EXISTS idx_voters_part ON voters(part_number)'); } catch {}
  try { _db.exec('ALTER TABLE voters ADD COLUMN name_hindi TEXT'); } catch {}
  try { _db.exec('CREATE INDEX IF NOT EXISTS idx_voters_name_hindi ON voters(name_hindi)'); } catch {}
...
async function seedDatabase() {
  ...
  const amitId   = _db.prepare("SELECT id FROM users WHERE phone='8888888001'").get()?.id;
  const deepakId = _db.prepare("SELECT id FROM users WHERE phone='8888888002'").get()?.id;
  insUser.run('Rahul Chauhan', '7777777001', workerPass, 'field_worker', 1, amitId);
  insUser.run('Vikram Joshi',  '7777777002', workerPass, 'field_worker', 1, amitId);
  insUser.run('Sanjay Negi',   '7777777003', workerPass, 'field_worker', 2, deepakId);
